...
import { Card, CardBody, Button, Textarea } from '@material-tailwind/react';
...
const QuickEvaluationOptimized: React.FC<QuickEvaluationOptimizedProps> = ({
  ...
  const [followups] = useState<Array<any>>([]);
  const [newFollowup, setNewFollowup] = useState('');
  const [behaviorComment, setBehaviorComment] = useState('');
  const [loading, setLoading] = useState(false);

  // حساب المجموع والمستوى
  const calculateTotalXP = (evalData: EvaluationData) => {
    const baseXP = (evalData.behavior_score + evalData.participation_score + evalData.notebook_score + 
                   evalData.attendance_score + evalData.portfolio_score) * 2; // كل نقطة = 2 XP
    const quranXP = evalData.quran_memorization * 10; // نقاط حفظ القرآن
    const bonusXP = evalData.bonus_points * 5; // نقاط إضافية
        ...
        <CardBody className="p-4 h-full flex flex-col">
          ...
          <div className="flex items-center justify-between mb-3 pb-2 border-b">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-right">
                <div className={`text-sm font-bold ${LEVEL_COLORS[evaluation.student_level as keyof typeof LEVEL_COLORS]}`}>
                  {LEVEL_NAMES[evaluation.student_level as keyof typeof LEVEL_NAMES]}
                </div>
          ...
          {sectionStudents && sectionStudents.length > 0 && (
            <div className="mb-3 p-2 bg-gray-50 rounded-lg border">
              <div className="text-xs text-gray-600 mb-1">طلاب القسم ({sectionStudents.length}):</div>
              ...
              <div className="h-full flex gap-6">
                ...
                <div className="w-80 space-y-4">
                  ...
                  <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white p-3 rounded-lg text-center">
                    ...
                    <div className="text-sm opacity-90">{LEVEL_NAMES[evaluation.student_level as keyof typeof LEVEL_NAMES]}</div>
                  </div>
                </div>
              </div>
            )}
