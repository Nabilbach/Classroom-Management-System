...
console.log('🔄 استعادة الحصص المفقودة (النسخة النهائية المصححة)\n');
...
function restoreMissingLessons() {
  return new Promise((resolve) => {
    ...
    backupDb.all(query, missingLessonIds, (err, missingLessons) => {
      ...
      // إدراج الحصص المفقودة بالأعمدة الصحيحة فقط
      const insertQuery = `
        INSERT INTO ScheduledLessons (
          id, date, startTime, assignedSections, completionStatus,
          customTitle, customDescription, subject, stages, estimatedSessions,
          manualSessionNumber, templateId, lessonGroupId, classroom, notes,
        ...
        if (insertedCount > 0) {
          ...
          currentDb.get('SELECT COUNT(*) as count FROM ScheduledLessons', (err, result) => {
            if (!err) {
              ...
              if (result.count === 12) {
                console.log('🎯 ممتاز! تم استعادة جميع الحصص المفقودة');
              }
            }
