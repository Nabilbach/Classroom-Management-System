...
import { Paper, Box, Typography, Button, TextField, Slider, IconButton, Tooltip } from '@mui/material';
import CloseIcon from '@mui/icons-material/Close';
import ChevronLeftIcon from '@mui/icons-material/ChevronLeft';
import ChevronRightIcon from '@mui/icons-material/ChevronRight';
// (أزيل استخدام الأكورديون واستبدل بتبويبات فرعية داخل التقييم)
// v2: 3-column compact layout, no scroll, all elements visible
// Updated: step=0.5 marks, portfolio→homework, defaults=0
...
function QuickEvaluation({ studentId, studentName, onClose, onSave, sectionStudents = [], onSwitchStudent }: QuickEvaluationProps) {
  ...
  const [evaluationSubTab, setEvaluationSubTab] = useState<'behavior' | 'participation' | 'notebook' | 'homework' | 'attendance' | 'quran' | 'bonus' | 'notes'>('behavior');
  // وضع الشبكة الكاملة لإظهار كل العناصر دون تمرير
  ...
  // بدءاً نجعل الشبكة اليمنى (أرقام التلاميذ) مطوية حتى يتوفر عرض وارتفاع أكبر لعناصر التقييم
  ...
  <div dir="rtl" className="flex gap-4 w-full max-w-full mx-auto px-6" style={{ alignItems: 'flex-start' }}>
      <div className="flex-1">
        ...
        {/* شريط أرقام الطلاب العلوي للوصول السريع */}
        {Array.isArray(sectionStudents) && sectionStudents.length > 0 && (
          <div className="mb-3 overflow-x-auto hide-scrollbar" style={{ WebkitOverflowScrolling: 'touch' }}>
            <div className="flex gap-1 min-w-max pr-1">
              {sectionStudents.slice(0,40).map((s:any) => {
                ...
                return (
                  <Tooltip key={s.id} title={`${s.firstName ?? ''} ${s.lastName ?? ''}`.trim()}>
                    <button
                      ...
                      className={`px-2 py-1 rounded text-xs font-semibold border transition ${isCurrent ? 'bg-blue-600 text-white border-blue-600' : 'bg-white hover:bg-blue-50 text-gray-700 border-gray-300'}`}
                      style={{ minWidth: 34 }}
                    >
                      {num}
                    </button>
  ...
  {/* Tabs (التبويبات الرئيسية) */}
        <div className="mb-2">
          <div className="flex gap-2 flex-wrap">
            <button className={`px-3 py-1 rounded text-sm ${activeTab === 'evaluation' ? 'bg-blue-600 text-white' : 'bg-gray-100'}`} onClick={() => setActiveTab('evaluation')}>التقييم</button>
            <button className={`px-3 py-1 rounded text-sm ${activeTab === 'followups' ? 'bg-blue-600 text-white' : 'bg-gray-100'}`} onClick={() => setActiveTab('followups')}>المتابعات</button>
            <button className={`px-3 py-1 rounded text-sm ${activeTab === 'history' ? 'bg-blue-600 text-white' : 'bg-gray-100'}`} onClick={() => { setActiveTab('history'); loadAssessments(); }}>سجل التقييم</button>
          </div>
        </div>
  ...
  {activeTab === 'evaluation' && (
    <div className="space-y-2" style={{ display: 'flex', flexDirection: 'column' }}>
      <div className="flex items-center justify-between gap-2">
        <div className="text-xs text-gray-500 font-medium">وضع العرض: {fullView ? 'شبكة كاملة' : 'تبويبات'}</div>
        <div className="flex gap-2">
          <Button size="small" variant={fullView?'contained':'outlined'} onClick={()=>setFullView(true)}>عرض كامل</Button>
          <Button size="small" variant={!fullView?'contained':'outlined'} onClick={()=>setFullView(false)}>تبويبات</Button>
        </div>
      </div>
        ...
        <div className="border rounded p-3 bg-white">
          <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))' }}>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-semibold"><span>😊 السلوك</span><span className="text-blue-600">{evaluation.behavior_score.toFixed(1)}/10</span></div>
              <Slider min={0} max={10} step={0.5} value={Number(evaluation.behavior_score)} onChange={(e,v)=>handleSliderChange('behavior_score',e,v)} size="small" />
              <div className="text-[10px] text-gray-500 leading-tight">{getBehaviorText(evaluation.behavior_score)}</div>
            </div>
            {/* المشاركة */}
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-semibold"><span>🗣️ المشاركة</span><span className="text-blue-600">{evaluation.participation_score.toFixed(1)}/10</span></div>
              <Slider min={0} max={10} step={0.5} value={Number(evaluation.participation_score)} onChange={(e,v)=>handleSliderChange('participation_score',e,v)} size="small" />
            </div>
            {/* الدفتر */}
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-semibold"><span>📝 الدفتر</span><span className="text-blue-600">{evaluation.notebook_score.toFixed(1)}/10</span></div>
              <Slider min={0} max={10} step={0.5} value={Number(evaluation.notebook_score)} onChange={(e,v)=>handleSliderChange('notebook_score',e,v)} size="small" />
            </div>
            {/* القيام بالواجبات */}
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-semibold"><span>📋 القيام بالواجبات</span><span className="text-blue-600">{evaluation.portfolio_score.toFixed(1)}/10</span></div>
              <Slider min={0} max={10} step={0.5} value={Number(evaluation.portfolio_score)} onChange={(e,v)=>handleSliderChange('portfolio_score',e,v)} size="small" />
            </div>
            {/* الحضور */}
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-semibold"><span>📅 الحضور</span><span className="text-blue-600">{evaluation.attendance_score.toFixed(1)}/10</span></div>
              <Slider min={0} max={10} step={0.5} value={Number(evaluation.attendance_score)} onChange={(e,v)=>handleSliderChange('attendance_score',e,v)} size="small" />
            </div>
            {/* حفظ القرآن */}
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-semibold"><span>📿 حفظ القرآن</span><span className="text-green-600">{evaluation.quran_memorization}</span></div>
              <div className="grid grid-cols-2 gap-1">
                {[{ value:0,label:'لم يحفظ',reward:'0'},{ value:5,label:'جزئي',reward:'+50'},{ value:10,label:'كامل',reward:'+100'},{ value:15,label:'متقن',reward:'+150'}].map(opt => (
                  <Button key={opt.value} onClick={()=>updateScore('quran_memorization',opt.value)} variant={evaluation.quran_memorization===opt.value?'contained':'outlined'} color={evaluation.quran_memorization===opt.value?'success':'inherit'} size="small" sx={{p:0.5,minWidth:0,fontSize:10,lineHeight:1.1}}>{opt.label}</Button>
                ))}
              </div>
            </div>
            {/* أعمال مميزة */}
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-semibold"><span>⭐ أعمال مميزة</span><span className="text-amber-600">+{evaluation.bonus_points * 5} XP</span></div>
              <input type="number" className="border rounded p-1 w-full text-xs" value={Number(evaluation.bonus_points)} min={0} max={20} onChange={(e)=>{ const v=(e.target as HTMLInputElement).value; const num=v===''?0:Number(v); updateScore('bonus_points', Number.isNaN(num)?0:num); }} />
            </div>
            {/* ملاحظات (تمتد بعرض كامل) */}
            <div className="space-y-1 col-span-full">
              <div className="flex justify-between text-xs font-semibold"><span>📝 ملاحظات</span><span className="text-gray-400">اختياري</span></div>
              <TextField value={evaluation.notes} onChange={(e)=>setEvaluation({...evaluation, notes:e.target.value})} placeholder="اكتب ملاحظات..." rows={2} multiline fullWidth size="small" />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center text-[11px] mt-3">
            <div className="p-2 border rounded bg-gray-50">مستوى: <span className="font-bold">{evaluation.student_level}</span></div>
            <div className="p-2 border rounded bg-gray-50">XP: <span className="font-bold">{evaluation.total_xp}</span></div>
            <div className="p-2 border rounded bg-gray-50">آخر تقييم: <span className="font-medium">{lastAssessmentDate? formatDateShort(lastAssessmentDate) : '—'}</span></div>
          </div>
          <div className="flex justify-end gap-2 pt-2 mt-2 border-t">
            <Button variant="outlined" color="inherit" onClick={onClose}>إغلاق</Button>
            <Button color="error" onClick={resetToZero} variant="outlined">إعادة للصفر</Button>
            <Button color="success" onClick={handleSave} disabled={saving} variant="contained">{saving?'جارٍ الحفظ...':'حفظ'}</Button>
          </div>
        </div>
      ) : (
        <>
          {/* التبويبات الفرعية */}
          <div className="flex flex-wrap gap-1 border rounded p-1 bg-gray-50">
            {[
              {id:'behavior',label:'السلوك',value:evaluation.behavior_score},
              {id:'participation',label:'المشاركة',value:evaluation.participation_score},
              {id:'notebook',label:'الدفتر',value:evaluation.notebook_score},
              {id:'homework',label:'القيام بالواجبات',value:evaluation.portfolio_score},
              {id:'attendance',label:'الحضور',value:evaluation.attendance_score},
              {id:'quran',label:'حفظ القرآن',value:evaluation.quran_memorization},
              {id:'bonus',label:'أعمال مميزة',value:evaluation.bonus_points},
              {id:'notes',label:'ملاحظات',value:0}
            ].map(t => (
              <button
                key={t.id}
                onClick={() => setEvaluationSubTab(t.id as any)}
                className={`px-2 py-1 rounded text-xs font-semibold border transition ${evaluationSubTab===t.id ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white hover:bg-indigo-50 text-gray-700 border-gray-300'}`}
              >
                {t.label}{['quran','bonus','notes'].includes(t.id)?'':(typeof t.value === 'number' ? ` (${(t.value as number).toFixed(1)})` : ` (${t.value})`)}
              </button>
            ))}
          </div>
          <div className="border rounded p-3 bg-white" style={{ minHeight: 150 }}>
            {evaluationSubTab === 'behavior' && (
              <div>
                <div className="flex justify-between mb-2 text-sm font-semibold"><span>😊 السلوك</span><span className="text-blue-600">{evaluation.behavior_score.toFixed(1)}/10</span></div>
                <Slider min={0} max={10} step={0.5} marks value={Number(evaluation.behavior_score)} onChange={(e,v)=>handleSliderChange('behavior_score',e,v)} />
                <div className="mt-2 text-xs text-gray-600">{getBehaviorText(evaluation.behavior_score)}</div>
              </div>
            )}
            {evaluationSubTab === 'participation' && (
              <div>
                <div className="flex justify-between mb-2 text-sm font-semibold"><span>🗣️ المشاركة</span><span className="text-blue-600">{evaluation.participation_score.toFixed(1)}/10</span></div>
                <Slider min={0} max={10} step={0.5} marks value={Number(evaluation.participation_score)} onChange={(e,v)=>handleSliderChange('participation_score',e,v)} />
              </div>
            )}
            {evaluationSubTab === 'notebook' && (
              <div>
                <div className="flex justify-between mb-2 text-sm font-semibold"><span>📝 الدفتر</span><span className="text-blue-600">{evaluation.notebook_score.toFixed(1)}/10</span></div>
                <Slider min={0} max={10} step={0.5} marks value={Number(evaluation.notebook_score)} onChange={(e,v)=>handleSliderChange('notebook_score',e,v)} />
              </div>
            )}
            {evaluationSubTab === 'homework' && (
              <div>
                <div className="flex justify-between mb-2 text-sm font-semibold"><span>📋 القيام بالواجبات</span><span className="text-blue-600">{evaluation.portfolio_score.toFixed(1)}/10</span></div>
                <Slider min={0} max={10} step={0.5} marks value={Number(evaluation.portfolio_score)} onChange={(e,v)=>handleSliderChange('portfolio_score',e,v)} />
              </div>
            )}
            {evaluationSubTab === 'attendance' && (
              <div>
                <div className="flex justify-between mb-2 text-sm font-semibold"><span>📅 الحضور</span><span className="text-blue-600">{evaluation.attendance_score.toFixed(1)}/10</span></div>
                <Slider min={0} max={10} step={0.5} marks value={Number(evaluation.attendance_score)} onChange={(e,v)=>handleSliderChange('attendance_score',e,v)} />
              </div>
            )}
            {evaluationSubTab === 'quran' && (
              <div>
                <div className="flex justify-between mb-2 text-sm font-semibold"><span>📿 حفظ القرآن</span><span className="text-green-600">{evaluation.quran_memorization}</span></div>
                <div className="grid grid-cols-4 gap-2">
                  {[{ value:0,label:'لم يحفظ',reward:'0 XP'},{ value:5,label:'جزئي',reward:'+50'},{ value:10,label:'كامل',reward:'+100'},{ value:15,label:'متقن',reward:'+150'}].map(opt => (
                    <Button key={opt.value} onClick={()=>updateScore('quran_memorization',opt.value)} variant={evaluation.quran_memorization===opt.value?'contained':'outlined'} color={evaluation.quran_memorization===opt.value?'success':'inherit'} size="small" sx={{p:0.75,minWidth:0,fontSize:11,lineHeight:1.1}}>
                      <div className="text-center leading-tight">
                        <div>{opt.label}</div>
                        <div className="text-[10px]">{opt.reward}</div>
                      </div>
                    </Button>
                  ))}
                </div>
            ...
            {evaluationSubTab === 'bonus' && (
              <div>
                <div className="flex justify-between mb-2 text-sm font-semibold"><span>⭐ أعمال مميزة</span><span className="text-amber-600">+{evaluation.bonus_points * 5} XP</span></div>
                <input type="number" className="border rounded p-2 w-full text-sm" value={Number(evaluation.bonus_points)} min={0} max={20} onChange={(e)=>{ const v=(e.target as HTMLInputElement).value; const num=v===''?0:Number(v); updateScore('bonus_points', Number.isNaN(num)?0:num); }} />
              </div>
            )}
            {evaluationSubTab === 'notes' && (
              <div>
                <div className="flex justify-between mb-2 text-sm font-semibold"><span>📝 ملاحظات</span><span className="text-gray-500 text-xs">اختياري</span></div>
                <TextField value={evaluation.notes} onChange={(e)=>setEvaluation({...evaluation, notes:e.target.value})} placeholder="اكتب ملاحظات..." rows={4} multiline fullWidth size="small" />
              </div>
            )}
          </div>
  ...
  <div className="hidden mt-4 p-2 bg-gray-50 rounded text-sm">
          ...
          <div className="truncate">payload: {JSON.stringify({ new_score: Number(((((evaluation.behavior_score + evaluation.participation_score + evaluation.notebook_score + evaluation.attendance_score + evaluation.portfolio_score) / 5) * 2)).toFixed(2)), scores: {
            ...
            {/* الأزرار انتقلت داخل تبويب التقييم */}
            {lastAssessmentDate && (
